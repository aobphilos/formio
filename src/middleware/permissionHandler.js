...
module.exports = function (router) {
  ...
  const getSubmissionResourceAccess = function (req, submission, access, next) {
    ...
    async.each(
      ...
      function (err) {
        ...
        access.submission.read_all = _(access.submission.read_all)
          .uniq()
          .value();
        access.submission.create_all = _(access.submission.create_all)
          .uniq()
          .value();
        access.submission.update_all = _(access.submission.update_all)
          .uniq()
          .value();
        access.submission.delete_all = _(access.submission.delete_all)
          .uniq()
          .value();
        ...
        if (
          _.intersection(req.submissionResourceAccessAdminBlock, adminAccess)
            .length
        ) {
      ...
      if (permissions[permissionLevel]) {
        permissions[permissionLevel].forEach((right) => {
          if (access.submission[right]) {
            ...
            access.submission[right] = _(access.submission[right])
              .uniq()
              .value();
          }
        });
      ...
      ([permissionLevel, conditions]) => {
        ...
        conditions.forEach((condition) => {
          ...
          if (rolesIntersection.length) {
            ...
            if (
              isConditionMet(
                util.castValue(valueType, value),
                formFieldValue,
                operator
              )
            ) {
  ...
  const getSelfAccessPermissions = function (req, form, access) {
    if (
      ...
      !(form.submissionAccess instanceof Array)
    ) {
      return;
    }
  ...
  router.formio.access = {
        ...
        hook.alter(
          ...
          [
                ...
                function (err, item) {
                  ...
                  if (item.submissionAccess) {
                    item.submissionAccess.forEach(function (permission) {
                      // Define this access type.
                      access.submission[permission.type] =
                        access.submission[permission.type] || [];
                      ...
                      if (permission.type === "update_all") {
                        access.submission.create_all =
                          access.submission.create_all || []; //eslint-disable-line camelcase
                      }
            ...
            function getAccessRoles(callback) {
              // Load all the roles.
              router.formio.resources.role.model.find(
                hook.alter(
                  "roleQuery",
                  {
                    deleted: { $eq: null },
            ...
            function setSubmissionFieldMatchAccessToRequest(callback) {
              ...
              router.formio.cache.loadForm(
                ...
                function (err, item) {
                  ...
                  if (
                    item.fieldMatchAccess &&
                    !_.isEmpty(item.fieldMatchAccess)
                  ) {
            ...
            function flagIndexRequestAsSubmissionFieldMatchAccess(callback) {
              ...
              if (
                req.submissionFieldMatchAccess &&
                _.isObject(req.submissionFieldMatchAccess)
              ) {
                const hasRoles = Object.keys(
                  req.submissionFieldMatchAccess
                ).some((accessKey) => {
                  if (
                    !Array.isArray(req.submissionFieldMatchAccess[accessKey])
                  ) {
                    return false;
                  }
                  return req.submissionFieldMatchAccess[accessKey].some(
                    (item) => {
                      return item.roles.some((role) =>
                        req.accessRoles.includes(role.toString())
                      );
                    }
                  );
                });
    ...
    hasAccess(req, access, entity, res) {
      ...
      const isIndex =
        method === "GET" && entity.hasOwnProperty("id") && entity.id === "";
      const isPost =
        method === "POST" && entity.hasOwnProperty("id") && entity.id === "";
      ...
      if (hasAllAccess) {
        const submissionResourceAdmin =
          _.get(req, "submissionResourceAccessAdminBlock") || [];
        if (
      ...
      if (
        (_.has(req, "submissionResourceAccessFilter") &&
          req.submissionResourceAccessFilter) ||
  ...
  return function permissionHandler(req, res, next) {
    ...
    router.formio.access.getAccess(req, res, function (err, access) {
      ...
      if (
        ...
        req.hasOwnProperty("formId") &&
        req.formId !== null &&
        req.formId !== undefined
      ) {
        entity = {
          type: "form",
          id: req.formId,
