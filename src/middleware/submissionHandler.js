...
module.exports = (router, resourceName, resourceId) => {
  ...
  [
    ...
    function loadCurrentForm(req, done) {
      router.formio.cache.loadCurrentForm(
        req,
        (err, form) => {
          if (err) {
            return done(err);
          ...
          router.formio.cache.loadSubForms(req.currentForm, req, () => {
            req.flattenedComponents = util.flattenComponents(
              form.components,
              true
            );
            return done();
    ...
    function initializeSubmission(req, done) {
      ...
      if (isGet) {
        const submissionQuery = hook.alter(
          "submissionQuery",
          { form: req.currentForm._id },
          req
        );
      ...
      if (!isGet && req.body) {
        ...
        if (
          req.body.hasOwnProperty("_fvid") &&
          !_.isNaN(parseInt(req.body._fvid))
        ) {
    ...
    function validateSubmission(req, res, done) {
      ...
      let isSubform =
        formId && formId.toString() !== req.currentForm._id.toString();
      isSubform =
        ...
        async (form) => {
          ...
          validator.validateReCaptcha = (responseToken) => {
            return new Promise((resolve, reject) => {
              router.formio.mongoose.models.token.findOne(
                { value: responseToken },
                (err, token) => {
                  if (err) {
                    return reject(err);
                  ...
                  if (!token) {
                    return reject(
                      new Error("ReCaptcha: Response token not found")
                    );
                  }
          ...
          validator.validate(req.body, (err, data, visibleComponents) => {
            ...
            if (!_.isEqual(visibleComponents, req.currentForm.components)) {
              ...
              req.hasOwnProperty("currentFormComponents") &&
              req.currentFormComponents
            ) {
    ...
    function executeFieldHandlers(validation, req, res, done) {
      ...
      util.eachValue(
        req.currentFormComponents || req.currentForm.components,
        submissionData,
        ({ component, data, handler, action, path }) => {
          if (component) {
            ...
            if (
              ...
              !["columns", "fieldset", "panel", "table", "tabs"].includes(
                component.type
              )
            ) {
              ...
              const newCompData = _.get(
                submissionData,
                componentPath,
                undefined
              );
              const currentCompData = _.get(
                req.currentSubmissionData,
                componentPath
              );
              ...
              if (
                ...
                _.set(
                  submissionData,
                  componentPath,
                  _.get(req.currentSubmissionData, componentPath)
                );
              }
            }
